...
export type NaverSearchCoverage = { received: number; valid: number; inBounds: number; matched: number };
/** NAVER search uses a private client secret and runs only on our server. */
export async function fetchNaverDatePlaces(
  query: string, options: { endpoint: string; token: string; region?: string; focus?: boolean; bounds?: MapBounds; signal?: AbortSignal; onCoverage?: (coverage: NaverSearchCoverage) => void },
  ...
  const coverage: NaverSearchCoverage = { received: payload.results.length, valid: 0, inBounds: 0, matched: 0 };
