...
export function ChatPage({ Header, messages, setMessages }: { Header: ({ title }: { title?: string }) => React.ReactNode; messages: Message[]; setMessages: React.Dispatch<React.SetStateAction<Message[]>> }) {
  ...
  return <div className="page full-page chat-page"><Header title="대화" /><div className="chat-profile"><div className="character-avatar"><Character kind="i" size="avatar" /></div><div><b>서연</b><span><i /> 지금 함께 있어요</span></div><button aria-label="대화 메뉴"><MoreHorizontal /></button></div><div className="messages" onClick={() => active && setActive(undefined)}>{messages.length === 0 && <div className="character-empty chat-empty"><Character mood="miss" size="medium" /><h3>우리의 첫 이야기를<br />시작해볼까요?</h3><p>메시지를 보내보세요 💜</p></div>}{messages.map((message, index) => { const date = new Date(message.timestamp).toDateString(); const previousDate = index > 0 ? new Date(messages[index - 1].timestamp).toDateString() : ''; const divider = date !== previousDate; return <div key={message.id}>{divider && <div className="date-chip">{messageDateLabel(message.timestamp)}</div>}<ChatBubble message={message} reply={message.replyTo ? byId.get(message.replyTo) : undefined} active={active === message.id} highlighted={highlighted === message.id} onAction={() => setActive(active === message.id ? undefined : message.id)} onReact={(emoji) => react(message.id, emoji)} onReply={() => { setReplyTo(message.id); setActive(undefined); }} onSave={() => { setMessages((items) => items.map((item) => item.id === message.id ? { ...item, saved: !item.saved } : item)); setActive(undefined); }} onImage={setLightbox} onJump={jump} /></div>; })}<div ref={bottomRef} /></div><ChatComposer draft={draft} reply={replyTo ? byId.get(replyTo) : undefined} onDraft={setDraft} onSend={send} onImage={sendImage} onCancelReply={() => setReplyTo(undefined)} />{lightbox && <div className="lightbox" role="dialog" onClick={() => setLightbox(undefined)}><button aria-label="닫기"><X /></button><img src={lightbox} alt="확대된 채팅 사진" /></div>}</div>;
}
