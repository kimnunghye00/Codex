...
export function MemoryForm({ memory, onSave, onClose }: { memory?: Memory; onSave: (memory: Memory) => void; onClose: () => void }) {
  ...
  return <div className="sheet-backdrop" role="dialog" aria-modal="true"><form className="memory-form" onSubmit={submit}><header><div><small>{memory ? 'EDIT MEMORY' : 'NEW MEMORY'}</small><h2>{memory ? '추억 수정' : '추억 추가'}</h2></div><button type="button" onClick={onClose} aria-label="닫기"><X /></button></header><label>제목<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="어떤 순간이었나요?" required /></label><label>날짜<input type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></label><label className="photo-field">사진<input type="file" accept="image/*" multiple onChange={(event) => void addImages(event.target.files)} /><span><ImagePlus size={18} />사진 선택 ({images.length})</span></label>{!!images.length && <div className="form-thumbnails">{images.map((image, index) => <img src={image} alt={`선택 사진 ${index + 1}`} key={`${image.slice(0, 30)}-${index}`} />)}</div>}<label>설명<textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="그날의 이야기를 남겨주세요" rows={3} /></label><label>장소<input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="예: 서울숲" /></label><label>태그<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="여행, 산책 (쉼표로 구분)" /></label><button className="primary" type="submit" disabled={!title.trim() || !images.length}>{memory ? '변경사항 저장' : '추억 저장'}</button></form></div>;
}
