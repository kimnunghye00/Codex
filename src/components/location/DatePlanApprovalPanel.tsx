...
export function DatePlanApprovalPanel({ coupleId, planId, uid, candidates, approval, loading, draftReady, detailsReady, onRefreshApproval }: Props) {
  ...
  return <section className="date-plan-approval-panel" aria-label="데이트 공동 승인">
    ...
    {!loading && !approval && <>
      <p className="date-map-add-hint">데이트 이름과 일정을 저장한 뒤 승인 요청을 보내세요. 날짜와 시작 시각은 미정이어도 괜찮아요. 요청하면 현재 일정이 잠기고 상대방의 승인을 기다려요.</p>
      ...
      {(!draftReady || !detailsReady) && <small>데이트 이름과 일정을 입력하고 시간표 자동 저장이 완료되어야 요청할 수 있어요.</small>}
    </>}
    {!loading && approval && <>
