...
export function AccountSettings({ user, profile, onProfileChange, onClose }: {
  ...
  const letAiChooseNickname = () => {
    ...
    syncProfile(next);
    setNickname(chosen);
    setAiFeedback(`ROUTE가 "${chosen}"라고 별명을 지어줬어요. 이 변경은 내 월 1회 변경 횟수를 사용하지 않아요.`);
  };
  ...
  return <>
    <div className="sheet-backdrop account-backdrop" role="dialog" aria-modal="true" aria-label="계정 설정">
      <div className="account-settings profile-enabled-settings">
        ...
        <section className="ai-test-card real-couple-connect-card">
          <div><Link2 size={20} /><span><strong>상대방과 연결하기</strong><small>초대 코드를 만들거나 상대방에게 받은 코드를 입력해 실제 계정을 연결해요.</small></span></div>
          <button type="button" className="nickname-edit-button" onClick={() => setCoupleConnectOpen(true)}>상대방 연결 설정 열기</button>
        </section>
        ...
        <section className="ai-test-card">
          <div><Bot size={20} /><span><strong>AI 테스트 파트너</strong><small>실제 상대방 기능을 만들기 전에 ROUTE의 커플 동작을 함께 테스트해요.</small></span></div>
          <button type="button" className="nickname-edit-button" disabled={aiBusy || Boolean(aiPartner?.connected)} onClick={() => void connectAi()}>{aiBusy ? '연결 중...' : aiPartner?.connected ? 'AI 파트너 연결됨' : 'AI 테스트 파트너 연결'}</button>
          {aiPartner?.connected && <button type="button" className="nickname-edit-button ai-nickname-button" onClick={letAiChooseNickname}><Sparkles size={15} />AI가 내 별명 지어주기</button>}
    ...
    {coupleConnectOpen && <div className="sheet-backdrop account-backdrop couple-connect-settings-backdrop" role="dialog" aria-modal="true" aria-label="상대방 연결 설정">
      <button className="standard-close-button couple-settings-close" type="button" aria-label="닫기" onClick={() => setCoupleConnectOpen(false)}><X /></button>
      <CoupleConnect
        user={user}
        profile={profile}
        onConnected={() => {
          setCoupleConnectOpen(false);
          setFeedback('상대방 계정과 연결됐어요.');
        }}
      />
    </div>}
  </>;
